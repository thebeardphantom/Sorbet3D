...
namespace sorbengine::ecs::components
{
	glm::mat4 camera::get_view_matrix(const transform& tform) const
	{
		...
		const auto translation_matrix = glm::translate(smath::identity_matrix, -tform.local_position);
		return inverse_rotation_matrix * translation_matrix;
	}

	glm::mat4 camera::get_perspective_matrix() const
	{
		auto projection_matrix = glm::perspective(glm::radians(fov), 800.0f / 600.0f, 0.1f, 100.0f);
